...
//1st section- COD, total price, no. of items
//2nd box-Personal details form-(inside section) name, email, number, addres, country,state, zipcode, aggree to T&C, Place order button
...
const CheckoutPage = () => {
    ...
    //above 2 same as CartPage
    const {  currentUser} =  useAuth(); //u can just do =true initially, 
    //take react hook form from Login 
    const {
        register,
        handleSubmit,
        watch,
    ...
    const navigate =  useNavigate()  //for when order is successful

    const [isChecked, setIsChecked] = useState(false)
    const onSubmit = async (data) => {
     //to place order on submit- new object created
        const newOrder = {
            name: data.name,
        ...
        //alert when order is placed, take from sweeatalert2site
        try {
            await createOrder(newOrder).unwrap();
            //taking swal.fire from sweetaleart2 website
            Swal.fire({
